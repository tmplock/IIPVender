...
let GetGameURL = async (strAgentCode, strID, strSecretCode, strGameKey, strReturnURL) => {
    ...
    if ( cToken != '' )
    {
        ...
        const bResult = await IHelper.UpdateToken(strAgentID, cToken, cToken, strAgentCode, strSecretCode, strReturnURL);
        if ( bResult == true )
        {
            try {
...
router.get('/player/check/:account', async (req, res) => {
    ...
    else
    {
        ...
        const user = await IHelper.GetUserFromID2(IAccount.cVender, req.params.account);
...
router.post('/transaction/game/rollout', async (req, res) => {
    ...
    else
    {
        ...
        const transaction = await IHelper.FindDB(req.body.mtcode);
        if ( transaction != null )
        {
            iBalance = transaction.iBalance;
            eErrorCode = EErrorCode.eSuccess;
            objectRet = {
                "balance":parseFloat(iBalance),
                "currency":IAccount.cCurrency
        ...
        else
        {
            ...
            {
                const processbet = await IHelper.ProcessBet2(req.body.account, eType, IAccount.cVender, req.body.gamehall, '', req.body.roundid, req.body.amount, 0, req.body.eventTime, req.body.mtcode, 0);
...
router.post('/transaction/game/rollin', async (req, res) => {
    ...
    else
    {
        ...
        //const tokenuser = await IHelper.GetUserFromID2(IAccount.cVender, req.body.account);
        const tokenuser = await IHelper.GetUser(req.body.account);
        if ( tokenuser == null )
        {
            eErrorCode = EErrorCode.ePlayerNotFound;
        }
        else
        {
            ...
            else
            {
                ...
                const processwin = await IHelper.ProcessWin2(req.body.account, eType, IAccount.cVender, req.body.gamehall, '', req.body.roundid, cWin, 0, req.body.createTime, req.body.mtcode, 0);
                if(processwin == null)
                {
                    eErrorCode = EErrorCode.eSystemError;
                }
...
router.post('/transaction/game/debit', async (req, res) => {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                const processbet = await IHelper.ProcessBet2(req.body.account, eType, IAccount.cVender, req.body.gamehall, '', req.body.roundid, req.body.amount, 0, '', req.body.mtcode, 0);
                if(processbet == null)
                {
                    eErrorCode = EErrorCode.eSystemError;
                }
...
router.post('/transaction/game/credit', async (req, res) => {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                const processwin = await IHelper.ProcessWin2(req.body.account, eType, IAccount.cVender, req.body.gamehall, '', req.body.roundid, req.body.amount, 0, req.body.gamecode, req.body.mtcode, 0);
                if(processwin == null)
                {
                    eErrorCode = EErrorCode.eSystemError;
                }
...
router.post('/transaction/user/payoff', async (req, res) => {
    ...
    {
        //const tokenuser = await db.Tokens.findOne({where:{strID:req.body.account}});
        const tokenuser = IHelper.GetUser(req.body.account);
        if ( tokenuser == null )
        {
            eErrorCode = EErrorCode.ePlayerNotFound;
        }
        else
        {
            ...
            {
                const processwin = await IHelper.ProcessWin2(req.body.account, eType, IAccount.cVender, 'CQ9PayOff', '', req.body.promoid, req.body.amount, 0, req.body.remark, req.body.mtcode, 0);
                if(processwin == null)
                {
                    eErrorCode = EErrorCode.eSystemError;
                }
...
router.post('/transaction/game/refund', async (req, res) => {
    ...
    else
    {
        ...
        else
        {
            ...
            const processcancel = await IHelper.ProcessCancel2(strAgentID, req.body.mtcode);
